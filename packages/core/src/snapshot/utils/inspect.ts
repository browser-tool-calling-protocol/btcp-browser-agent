/**
 * @btcp/core - Snapshot Inspect Utilities
 *
 * Element inspection utilities for role detection, accessible name,
 * visibility checking, and xpath generation.
 */
 ...
 */
export function getHTMLConstructors(element: Element) {
 ...
 */
export function isInViewport(element: Element, window: Window): boolean {
 ...
 */
export function getInputAttributes(element: Element): string {
  const constructors = getHTMLConstructors(element);
 ...
 */
export function buildSemanticXPath(element: Element): string {
  const parts: string[] = [];
 ...
 */
export function getSectionName(element: Element): string {
